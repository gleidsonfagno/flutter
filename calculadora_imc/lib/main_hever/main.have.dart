...
class _IMCCalculatorScreenState extends State<IMCCalculatorScreen> {
  ...
  late Box<IMCModel> imcBox;

  void openHiveBox() async {
    imcBox = await Hive.openBox<IMCModel>('imcBox');
    imcList.addAll(imcBox.values);
  }
  ...
  void _calculateIMC() {
    ...
    if (weight > 0 && height > 0) {
      ...
      final imcModel = IMCModel(imc.weight, imc.height, imc.bmi);
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                final imcModel = imcList[index];
                final imc = IMC(imcModel.weight, imcModel.height);
                final classification = imc.calculateClassification();
                return ListTile(
                  title: Text('IMC: ${imcModel.bmi.toStringAsFixed(2)}'),
                  subtitle: Text(
                    'Peso: ${imcModel.weight.toStringAsFixed(2)} kg, '
                    'Altura: ${imcModel.height.toStringAsFixed(2)} m',
                  ),
                  trailing: Row(
